...
class EmailRetryService:
    ...
    async def _retry_worker_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                logger.error(
                    "Error in email retry worker loop",
                    error=str(e),
                    exc_info=True
                )
    ...
    async def _process_retry_queue(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "Processing email export retries",
                    count=len(retry_exports)
                )
    ...
    def _get_exports_for_retry(self, db: Session) -> List[EmailExport]:
        ...
        retry_exports = db.query(EmailExport).filter(
    ...
    async def _retry_single_export(self, db: Session, export: EmailExport) -> None:
        ...
        try:
            ...
            if export.retry_count >= self.max_retries:
                export.mark_as_failed(f"Maximum retry attempts reached. Last error: {str(e)}")
    ...
    async def retry_export_manually(
        ...
        # Get the export
        export = db.query(EmailExport).filter(
            EmailExport.id == export_id
        ).first()
        
        if not export:
            logger.warning(
                "Email export not found for manual retry",
                export_id=export_id
            )
            return False
    ...
    async def get_retry_statistics(self, db: Session) -> dict:
        ...
        pending_count = db.query(EmailExport).filter(
            EmailExport.status == 'pending'
        ).count()
        
        retry_count = db.query(EmailExport).filter(
            EmailExport.status == 'retry'
        ).count()
        
        sent_count = db.query(EmailExport).filter(
            EmailExport.status == 'sent'
        ).count()
        
        failed_count = db.query(EmailExport).filter(
            EmailExport.status == 'failed'
        ).count()
        
        # Get exports that have been retried
        retried_exports = db.query(EmailExport).filter(
            EmailExport.retry_count > 0
        ).all()
