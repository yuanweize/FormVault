...
class ApplicationCreateSchema(BaseModel):
    ...
    preferred_language: str = Field("en", max_length=5, description="Preferred language code")
...
class ApplicationUpdateSchema(BaseModel):
    ...
    personal_info: Optional[PersonalInfoSchema] = Field(None, description="Personal information")
    insurance_type: Optional[InsuranceType] = Field(None, description="Type of insurance")
    preferred_language: Optional[str] = Field(None, max_length=5, description="Preferred language code")
    student_id_file_id: Optional[str] = Field(None, description="Student ID file ID")
    passport_file_id: Optional[str] = Field(None, description="Passport file ID")
...
class ApplicationResponseSchema(ResponseBase, TimestampMixin):
    ...
    files: List[FileInfoSchema] = Field(default_factory=list, description="Uploaded files")
...
class ApplicationListResponseSchema(ResponseBase):
    ...
    applications: List[ApplicationResponseSchema] = Field(..., description="List of applications")


class ApplicationSubmitSchema(BaseModel):
    """Schema for submitting an application."""
    confirm_submission: bool = Field(..., description="Confirmation of submission intent")
