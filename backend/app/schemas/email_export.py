...
class EmailExportRequestSchema(BaseModel):
    ...
    insurance_company: Optional[str] = Field(
        None, max_length=255, description="Name of the insurance company"
    )

    additional_notes: Optional[str] = Field(
        None, max_length=1000, description="Additional notes to include in the email"
    )
...
class EmailExportResponseSchema(ResponseBase):
    ...
    export_id: str = Field(..., description="Unique identifier for the email export")

    application_id: str = Field(..., description="Application ID that was exported")
    ...
    status: str = Field(..., description="Current status of the email export")

    sent_at: Optional[datetime] = Field(
        None, description="Timestamp when the email was successfully sent"
    )
...
class EmailExportStatusSchema(BaseModel):
    ...
    error_message: Optional[str] = Field(
        None, description="Error message if the export failed"
    )
...
class EmailExportHistorySchema(ResponseBase):
    ...
    exports: list[EmailExportStatusSchema] = Field(
        default_factory=list, description="List of email exports for the application"
    )

    total_exports: int = Field(
        0, description="Total number of exports for this application"
    )
    ...
    pending_exports: int = Field(0, description="Number of pending exports")


class EmailExportRetryRequestSchema(BaseModel):
    """Schema for email export retry request."""

    export_id: str = Field(..., description="ID of the email export to retry")

    force_retry: bool = Field(
        False, description="Force retry even if max retries reached"
    )
...
class EmailExportRetryResponseSchema(ResponseBase):
    ...
    export_id: str = Field(..., description="ID of the email export that was retried")

    retry_attempted: bool = Field(..., description="Whether the retry was attempted")

    new_status: str = Field(..., description="New status after retry attempt")
