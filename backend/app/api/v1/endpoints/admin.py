...
async def reset_performance_statistics() -> BaseResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to reset performance statistics"
        )
...
async def _get_application_stats(
    ...
    type_counts = db.query(
        ...
        Application.created_at.between(start_date, end_date)
    ).group_by(Application.insurance_type).all()
    ...
    language_counts = db.query(
        ...
        Application.created_at.between(start_date, end_date)
    ).group_by(Application.preferred_language).all()
...
async def _get_file_stats(
    ...
    total_files = db.query(File).filter(
        File.created_at.between(start_date, end_date)
    ).count()
...
async def _get_email_stats(
    ...
    # Total email exports
    total_exports = db.query(EmailExport).filter(
    ...
    status_counts = db.query(
        EmailExport.status,
        func.count(EmailExport.id)
    ).filter(
        EmailExport.created_at.between(start_date, end_date)
    ).group_by(EmailExport.status).all()
    
    # Success rate
    successful_exports = db.query(EmailExport).filter(
        and_(
            EmailExport.created_at.between(start_date, end_date),
...
async def _get_activity_stats(
    ...
    # Unique IP addresses
    unique_ips = db.query(
        func.count(func.distinct(AuditLog.user_ip))
    ).filter(
        and_(
            AuditLog.created_at.between(start_date, end_date),
