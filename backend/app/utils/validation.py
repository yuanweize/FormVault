...
class InputValidator:
    ...
    def validate_name(name: str, field_name: str = "name") -> str:
        ...
        if len(name) < 1:
            raise ValidationError(
                f"{field_name} must be at least 1 character long", field=field_name
            )
    ...
    def validate_address_field(
        value: str, field_name: str, max_length: int = 100
    ) -> str:
    ...
    def validate_date_of_birth(date_value: Union[str, date, datetime]) -> date:
        ...
        if isinstance(date_value, str):
            try:
                ...
                try:
                    ...
                    raise ValidationError(
                        "Invalid date format. Use YYYY-MM-DD", field="date_of_birth"
                    )
        ...
        if age < 16:
            raise ValidationError(
                "Must be at least 16 years old", field="date_of_birth"
            )
    ...
    def validate_language(language: str) -> str:
        ...
        if not language:
            raise ValidationError(
                "Preferred language is required", field="preferred_language"
            )
        ...
        if not PATTERNS["language"].match(language):
            raise ValidationError(
                "Language must be one of: en, es, zh", field="preferred_language"
            )
    ...
    def validate_file_type(file_type: str) -> str:
        ...
        if not PATTERNS["file_type"].match(file_type):
            raise ValidationError(
                "File type must be one of: student_id, passport", field="file_type"
            )
...
def validate_address_info(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    validated_data = {
        "street": validator.validate_address_field(
            data.get("street", ""), "street", 200
        ),
        ...
        "country": validator.validate_address_field(
            data.get("country", ""), "country", 100
        ),
    }
