...
class TestAuditMiddleware:
    ...
    def test_determine_action_patterns(self):
        ...
        # Test exact matches
        assert (
            middleware._determine_action("POST", "/api/v1/applications")
            == "application.create"
        )
        assert (
            middleware._determine_action("GET", "/api/v1/applications")
            == "application.list"
        )

        # Test pattern matches
        assert (
            middleware._determine_action("GET", "/api/v1/applications/123/export")
            == "application.export"
        )
        assert (
            middleware._determine_action("PUT", "/api/v1/applications/123")
            == "application.update"
        )
    ...
    def test_extract_application_id_from_path(self):
        ...
        assert (
            middleware._extract_application_id("/api/v1/applications/invalid-id")
            is None
        )
