...
class TestEmailService:
    ...
    @pytest.mark.asyncio
    async def test_create_email_message(
        self, email_service_instance, sample_application
    ):
    ...
    def test_create_fallback_html_content(
        self, email_service_instance, sample_application
    ):
        ...
        html_content = email_service_instance._create_fallback_html_content(
            template_data
        )
    ...
    async def test_attach_application_files_no_files(
        self, mock_path, email_service_instance, sample_application
    ):
        """Test file attachment when no files exist."""
        sample_application.files = []
        message = MIMEMultipart()

        await email_service_instance._attach_application_files(
            message, sample_application
        )
    ...
    async def test_send_application_export_smtp_failure(
        self, mock_send_smtp, email_service_instance, sample_application
    ):
        ...
        with pytest.raises(EmailServiceException):
            await email_service_instance.send_application_export(
                application=sample_application, recipient_email="insurance@company.com"
            )

    @pytest.mark.asyncio
    async def test_retry_failed_export_max_retries_reached(
        self, email_service_instance
    ):
        ...
        result = await email_service_instance.retry_failed_export(
            mock_export, max_retries=3
        )
    ...
    async def test_retry_failed_export_success(
        self, mock_send_export, mock_sleep, email_service_instance
    ):
